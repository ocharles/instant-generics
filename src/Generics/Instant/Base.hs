...
data CEq k c a where C :: k => a -> CEq k c a
deriving instance (Show a) => Show (CEq k c a)
deriving instance (k, Read a) => Read (CEq k c a)
...
class Constructor c where
  conName   :: t (k :: Constraint) c a -> String
  {-# INLINE conFixity #-}
  conFixity :: t (k :: Constraint) c a -> Fixity
  conFixity = const Prefix
  {-# INLINE conIsRecord #-}
  conIsRecord :: t (k :: Constraint) c a -> Bool
